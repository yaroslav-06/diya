        ...
        return Center(
          child: CMatrixTransition(
              back: PassCard(
                  ...
                  scaleCh: scales[i]),
              front: PassCard(child: widget.children![i], scaleCh: scales[i])),
        );
      }),
    );
  }
...
class CMatrixTransition extends StatefulWidget {
  ...
  CMatrixTransition(
      {@required this.front,
      @required this.back,
      this.dur = const Duration(milliseconds: 16 * 600)});
  @override
  _MatrixTransitionState createState() => _MatrixTransitionState();
}

class _MatrixTransitionState extends State<CMatrixTransition>
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<double>(
      ...
      builder: (context, x, child) {
        return Transform.scale(scale: x, child: child);
      },
